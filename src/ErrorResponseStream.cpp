...
ErrorResponseStream::ErrorResponseStream(int status, HttpRequest &request)
...
void ErrorResponseStream::setContent() {
  ...
  *_content << "</head><body>\n<h1><b>" << _status << "</b> " << _message
            << "</h1>\n";
  *_content << "<hr>\n<address>ZiaHTTPD Server at http://"
            << this->_request.getHeaderOption("Host")
                   .substr(0, this->_request.getHeaderOption("Host").find(":"))
            << this->_request.getUri() << " Port ";
  if (this->_request.getHeaderOption("Host").find(":") != std::string::npos)
    *_content << this->_request.getHeaderOption("Host").substr(
                     this->_request.getHeaderOption("Host").find(":") + 1);
  else
...
ErrorResponseStream::~ErrorResponseStream() throw() { delete this->_content; }
...
bool ErrorResponseStream::completed() const { return this->_content->eof(); }

int ErrorResponseStream::getStatus() const { return this->_status; }

size_t ErrorResponseStream::getSize() const {
  return this->_content->str().length();
}

bool ErrorResponseStream::good() const { return true; }
